	...
	public class SpineBeginnerTwo : MonoBehaviour
	{
		...
		private IEnumerator DoDemoRoutine()
		{
			while (true)
			{
				...
				yield return new WaitForSeconds(runWalkDuration);
				spineAnimationState.SetAnimation(0, runToIdleAnimationName, loop: false);
				spineAnimationState.AddAnimation(0, idleAnimationName, loop: true, 0f);
				yield return new WaitForSeconds(1f);
				skeleton.FlipX = true;
				spineAnimationState.SetAnimation(0, idleTurnAnimationName, loop: false);
				spineAnimationState.AddAnimation(0, idleAnimationName, loop: true, 0f);
